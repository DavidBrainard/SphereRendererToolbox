...
diffuseConst = .33*ones(1,numSamples);
specularConst = .01*ones(1,numSamples);
specularBlurConst = .1;
lightScaleConst = 1; %for no light scaling, set to zero.
lightIntensity= ones(numSamples);

%set light parameters. 
%   here, each light is emulated by three lights,
%   one each for the R,G and B channels
%
%   this will allow channeled diffuse and specular reflectance
numLightSources = size(lightCoords,1);
% i=[1:numLightSources];
% i=repmat(i,[3 1]);
% i=reshape(i,[1 3*numLightSources]);
...
%find which pixels reflect light
display('      light reflectance...');
...
calc(17,i)=[diffuseConst*lightIntensity].*calc(13,i);                           
...
%reshape into bitmap image with one slice per light source
display('      adding color values...');
calc(19,:,:)=calc(17,:,:)+calc(18,:,:);
bitmapImageTemp=reshape(calc(19,:,:),[ROWS COLS numLightSources]);

%step through each light source, scale it three times for each of RGB
%and add to final bitmap image (will vectorize later)
lightColors = normalizeRow(lightColors);
for c=1:3
    for x=1:numLightSources
        bitmapImage(:,:,c)=bitmapImage(:,:,c)+lightColors(x,c)*bitmapImageTemp(:,:,x);
    end
end
